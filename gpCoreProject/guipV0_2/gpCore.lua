...
function MOD_gpCore.init()
    ...
    buttonClass = require("guipV0_2.guiElements.button")
    labelClass = require("guipV0_2.guiElements.label")
    cursorYClass = require("guipV0_2.guiElements.cursorY")
...
---@param x integer
---@param y integer
function MOD_gpCore.createLabel(text,x,y)

    table.insert(drawList,labelClass.new(text,x,y))
end

function MOD_gpCore.createButton(labelText,id,x,y,width,height,drawmode,color_)

    local labelOBJ = labelClass.newLabel(labelText,x,y)
    table.insert(drawList,buttonClass.new(x,y,width,height,labelOBJ,drawmode,id,color_))
end
